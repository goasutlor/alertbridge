...
const LANG = {
  en: {
    ...
    dailySub: "Incoming = webhook POSTs accepted. Fwd OK = webhooks where every outbound succeeded (including all unrolled alerts). When the target is healthy and delivery succeeds end-to-end, Incoming = Fwd OK. Fwd Fail / DLQ = one per webhook if any outbound failed; a day with no successes → Incoming = Fwd Fail = DLQ.",
    ...
    mapperCardSubtitle:
      "Map fields from source to target. Save stores one row per name in the library (saving again with the same name overwrites). Apply sets which preset is live for the selected route. Each target row has horizontal Source columns (1, 2, …) — same field list in every dropdown; first non-empty wins left to right. Custom paste uses the first JSON sample row for paths; you can tick OCP / Confluent to add preset paths.",
    mapperCustomPasteDesc: "Paste example incoming log JSON (one or more rows). Use as source fields builds the path list from the first non-empty row only (same field names / structure; values can differ). Add rows if you want to keep alternate payloads handy — only the first sample drives the dropdown. Optionally tick OCP / Confluent below to add preset paths.",
  ...
  th: {
    ...
    next: "ถัดไป",
    dailyTitle: "Daily Metrics",
    dailySub: "Incoming = จำนวน POST เข้า webhook — Fwd OK = จำนวน webhook ที่ส่งออกครบทุกครั้งสำเร็จ (รวมทุก alert หลัง unroll) — เมื่อเชื่อม target ได้และส่งครบตามปกติ Incoming จะเท่ากับ Fwd OK — Fwd Fail / DLQ = ต่อ webhook ที่มีการส่งล้มอย่างน้อยหนึ่งครั้ง — วันที่ไม่มีครั้งสำเร็จเลย Incoming = Fwd Fail = DLQ",
    ...
    mapperCardSubtitle:
      "Map จาก source ไป target — บันทึกได้หนึ่งแถวต่อชื่อ (บันทึกซ้ำชื่อเดิม = เขียนทับ); Apply กำหนดว่า route ไหนใช้ preset ไหนเป็นของจริง — แต่ละแถว target มีคอลัมน์ Source แนวนอน (1, 2, …) รายการ field เดียวกันทุก dropdown ค่าแรกที่ไม่ว่างจากซ้ายไปขวาชนะ — Custom ใช้แถว JSON แรกที่ไม่ว่างสร้าง path; ติ๊ก OCP/Confluent เพื่อเพิ่ม path จาก preset",
    mapperCustomPasteDesc: "วาง JSON ตัวอย่างได้หลายแถว — กด Use as source fields จะสร้างรายการ path จากแถวแรกที่ไม่ว่างเท่านั้น (โครง/ชื่อ field เดียวกัน ค่าต่างกันได้) — แถวอื่นเก็บไว้ดูได้ แต่ไม่เอามารวมรายการ path — ติ๊ก OCP/Confluent ด้านล่างเพื่อเพิ่ม path จาก preset",
